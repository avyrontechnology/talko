...
class TalkoUserRepository:
    ...
    async def count_users(self) -> int:
        try:
            async with self.db_manager.collection(TalkoUserModel.CollectionName.TALKO_USERS) as collection:
                return await collection.count_documents({})
        except Exception as e:
            self.logger.error(f"Failed to count talko users: {str(e)}")
            raise
    ...
    async def insert_user(self, doc: dict) -> str:
        try:
            ...
            self.logger.error(f"Failed to insert talko user: {str(e)}")
            raise
    ...
    async def find_by_email(self, email: str) -> dict | None:
        try:
            ...
            self.logger.error(f"Failed to find talko user by email: {str(e)}")
            raise
    ...
    async def find_by_id(self, user_id: str) -> dict | None:
        try:
            ...
            self.logger.error(f"Failed to find talko user by id: {str(e)}")
            raise
    ...
    async def list_users(self, limit: int = 100) -> list[dict]:
        try:
            ...
            self.logger.error(f"Failed to list talko users: {str(e)}")
            raise
    ...
    async def update_user(self, user_id: str, patch: dict) -> bool:
        try:
            ...
            self.logger.error(f"Failed to update talko user: {str(e)}")
            raise
