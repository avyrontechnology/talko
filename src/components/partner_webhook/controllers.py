...
class TalkoPartnerWebhookController:
    ...
    async def create_webhook_config(
        ...
        partner_webhook_service: TalkoPartnerWebhookService = Depends(Provide[TalkoContainer.partner_webhook_service]),
        ...
        try:
            ...
            talko_service_logger.error(f"Error creating webhook config: {str(e)}")
            return TalkoBadRequestResponse(detail=str(e))
        except Exception as e:
            talko_service_logger.error(f"Unexpected error creating webhook config: {str(e)}")
            return TalkoInternalServerErrorResponse(detail=SOMETHING_WENT_WRONG)

    @router.patch("/config/{partner_id}", response_model=TalkoContract.WebhookConfigResponse)
    @permission_check(TalkoPermissionDependency)
    @inject
    async def update_webhook_config(
        ...
        try:
            updated = await partner_webhook_service.update_webhook_config(partner_id, data)
            return TalkoSuccessResponse(data=updated)
        except ValueError as e:
            talko_service_logger.error(f"Error updating webhook config for partner_id {partner_id}: {str(e)}")
            return TalkoBadRequestResponse(detail=str(e))
        except Exception as e:
            talko_service_logger.error(
                f"Unexpected error updating webhook config for partner_id {partner_id}: {str(e)}"
            )
            return TalkoInternalServerErrorResponse(detail=SOMETHING_WENT_WRONG)

    @router.get("/config/{partner_id}", response_model=TalkoContract.WebhookConfigResponse)
    @permission_check(TalkoPermissionDependency)
    @inject
    async def get_webhook_config(
        ...
        try:
            ...
            talko_service_logger.error(f"Unexpected error getting webhook config for partner_id {partner_id}: {str(e)}")
            return TalkoInternalServerErrorResponse(detail=SOMETHING_WENT_WRONG)

    @router.get("/deliveries", response_model=list[TalkoContract.DeliveryAttemptResponse])
    @permission_check(TalkoPermissionDependency)
    @inject
    async def list_deliveries(
        ...
        try:
            attempts = await partner_webhook_service.list_delivery_attempts(partner_id, limit)
            return TalkoSuccessResponse(data=attempts)
        except Exception as e:
            talko_service_logger.error(
                f"Unexpected error listing webhook deliveries for partner_id {partner_id}: {str(e)}"
            )
            return TalkoInternalServerErrorResponse(detail=SOMETHING_WENT_WRONG)
