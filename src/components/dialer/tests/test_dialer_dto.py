...
class TestContractModels:
    ...
    def test_lead_list_item_success(self):
        ...
        item = Contract.LeadListItem(**data)
        assert item.id == 1
        assert item.name == "Test List"

    def test_lead_list_item_invalid_id(self):
        with pytest.raises(ValidationError) as exc:
            Contract.LeadListItem(id=0, name="Bad ID", description="desc", field_map=[])
    ...
    def test_lead_lists_fetch_response_success(self):
        ...
        resp = Contract.LeadListsFetchResponse(**response_data)
        assert resp.status == "success"
        assert len(resp.data["lists"]) == 1
        assert isinstance(resp.data["lists"][0], Contract.LeadListItem)
    ...
    def test_bulk_leads_create_success(self):
        ...
        req = Contract.BulkLeadsCreateRequest(**payload)
    ...
    def test_invalid_duplicate_option(self):
        ...
        with pytest.raises(ValidationError) as exc:
            Contract.BulkLeadsCreateRequest(**payload)
    ...
    def test_bulk_leads_create_response_success(self):
        resp = Contract.BulkLeadsCreateResponse(success=True, message="Leads uploaded")
