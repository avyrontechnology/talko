...
class _FakeDb(dict):
    """Mimics the motor DB from connect(): db[collection_name] -> collection."""
...
def _wire_connect(mock_db_manager, col):
    mock_db_manager.connect.return_value = TalkoAsyncContextManagerMock(_FakeDb(col))
...
class TestInsertDidDefaultAttendance:
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to insert DID attendance: Database error")
...
class TestInsertDidHistory:
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to insert DID history: Database error")
...
class TestFindDidAttendance:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        col.find_one.assert_awaited_once_with({"did_number": did_number, "partner_id": partner_id})
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to find DID attendance: Database error")
...
class TestDeleteDidDefaultAttendance:
    ...
    async def test_deleted(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.delete_did_default_attendance(did_number, partner_id)

        col.delete_one.assert_awaited_once_with({"did_number": did_number, "partner_id": partner_id})
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to delete DID attendance: Database error")
...
class TestUpdateDidHistory:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.update_did_history(did_number, partner_id, update_dict)
    ...
    @pytest.mark.asyncio
    async def test_not_found_returns_none(self, did_repository, mock_db_manager, mock_logger):
        col = AsyncMock()
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to update DID history: Database error")
...
class TestGetAssignedDids:
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to fetch assigned DIDs: Database error")
...
class TestGetAvailableDids:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        assert result == ["12345", "67890"]
        mock_logger.info.assert_called_with("Fetched available DIDs: ['12345', '67890']")
        mock_logger.error.assert_not_called()

    @pytest.mark.asyncio
    async def test_success_with_vendor_config_id(self, did_repository, mock_db_manager, mock_logger):
        vendor_id = ObjectId()
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to fetch available DIDs: Database error")
...
class TestFindDidByDidNumberAndVendorId:
    ...
    async def test_found(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.find_did_by_did_number_and_vendor_id(did_number, vendor_id)

        col.find_one.assert_awaited_once_with({"did_number": did_number, "vendor_id": vendor_id})
        assert result == expected
        mock_logger.debug.assert_any_call(f"Found DID {did_number} for vendor {vendor_id}")
        mock_logger.error.assert_not_called()
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        with pytest.raises(Exception, match="Database error"):
            await did_repository.find_did_by_did_number_and_vendor_id(did_number, vendor_id)

        mock_logger.error.assert_called_once_with(f"Unexpected error finding DID {did_number}: Database error")
...
class TestUpdateDidAttendance:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        _wire_connect(mock_db_manager, col)

        result = await did_repository.update_did_attendance(did_number, partner_id, update_data)

        col.find_one.assert_awaited_once_with({"did_number": did_number, "partner_id": 0})
        assert result == updated
        mock_logger.info.assert_called_with(f"Successfully updated DID {did_number} to partner {partner_id}")
        mock_logger.error.assert_not_called()
    ...
    async def test_not_available(self, did_repository, mock_db_manager, mock_logger):
        ...
        assert result is None
        mock_logger.warning.assert_called_once_with("DID 12345 not available (partner_id != 0)")

    @pytest.mark.asyncio
    async def test_update_returns_none(self, did_repository, mock_db_manager, mock_logger):
        col = AsyncMock()
        ...
        assert result is None
        mock_logger.warning.assert_called_with("Failed to update DID 12345 to partner 1")
    ...
    async def test_exception(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Unexpected error updating DID 12345: Database error")
...
class TestGetDidsByPartnerAndVendor:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.get_dids_by_partner_and_vendor(partner_id, vendor_id)
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to fetch DIDs for partner 1: Database error")
...
class TestGetDidsByPartnerWorkspaceAndVendor:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.get_dids_by_partner_workspace_and_vendor(partner_id, workspace_id, vendor_id)
        ...
        assert result == ["12345", "67890"]
        mock_logger.info.assert_called_with("Fetched DIDs for partner workspace and vendor: ['12345', '67890']")
        mock_logger.error.assert_not_called()
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        with pytest.raises(Exception, match="Database error"):
            await did_repository.get_dids_by_partner_workspace_and_vendor(1, 100, ObjectId())

        mock_logger.error.assert_called_once_with("Failed to fetch DIDs for partner 1, workspace 100: Database error")
...
class TestGetDidsByPartnerAgentWorkspaceAndVendor:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.get_dids_by_partner_agent_workspace_and_vendor(
        ...
        mock_logger.info.assert_called_with(
            "Fetched DIDs get did by partner agent workspace and vendor: ['12345', '67890']"
        )
        mock_logger.error.assert_not_called()
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        with pytest.raises(Exception, match="Database error"):
            await did_repository.get_dids_by_partner_agent_workspace_and_vendor(1, 200, 100, ObjectId())

        mock_logger.error.assert_called_once_with(
            "Failed to fetch DIDs for partner 1, agent 200, workspace 100: Database error"
        )
...
class TestGetDidByNumber:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        col.find_one.assert_called_once_with({"did_number": normalized, "is_active": True})
    ...
    @pytest.mark.asyncio
    async def test_not_found_logged_as_info(self, did_repository, mock_db_manager, mock_logger):
        call_to_number = "+911234567890"
        normalized = "911234567890"  # + stripped by normalize_phone_number with_plus=False
        col = AsyncMock()
        col.find_one = AsyncMock(return_value=None)
        _wire(mock_db_manager, col)
        ...
        col.find_one.assert_awaited_once_with({"did_number": normalized, "is_active": True})
        assert result is None
        mock_logger.debug.assert_any_call(f"Searching DID for number: {call_to_number}")
    ...
    async def test_with_partner_id(self, did_repository, mock_db_manager, mock_logger):
        ...
        col.find_one.assert_called_once_with({"did_number": normalized, "is_active": True, "partner_id": partner_id})
        assert result == did_record
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Error finding DID by number: DB error")
...
class TestGetDidsByWorkspace:
    ...
    async def test_failure(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to fetch DIDs for workspace_id=100: Database error")
...
class TestGetDetailsByDids:
    ...
    async def test_success_multiple(self, did_repository, mock_db_manager, mock_logger):
        ...
        assert result == expected
        mock_logger.info.assert_any_call(f"Fetching instance IDs for DIDs: {did_numbers}")
        mock_logger.info.assert_any_call(f"Fetched DID instance details: {expected}")
        mock_logger.error.assert_not_called()
...
class TestUnassignDidToPartner:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        assert result is True
        mock_logger.info.assert_any_call(f"Unassigning DiD {did_number} to partner_id: {partner_id}")
        mock_logger.info.assert_any_call(f"Successfully unassigned DID {did_number}")
        mock_logger.error.assert_not_called()
    ...
    async def test_no_update(self, did_repository, mock_db_manager, mock_logger):
        ...
        assert result is False
        mock_logger.warning.assert_called_once_with("No records updated for DID 1234567890")
    ...
    async def test_exception(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to fetch instance_id for DIDs 1234567890: DB error")
...
class TestUpdateDidStatus:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.update_did_status(did_number, partner_id, update_data)

        # find_one uses normalized number
        col.find_one.assert_awaited_once_with({"did_number": normalized, "partner_id": partner_id})
        assert result == updated
        mock_logger.info.assert_called_with(f"Successfully updated DID {did_number} to partner {partner_id}")
        mock_logger.error.assert_not_called()
    ...
    async def test_did_not_found(self, did_repository, mock_db_manager, mock_logger):
        ...
        result = await did_repository.update_did_status("9876543210", 1, {"status": "x"})

        assert result is None
        mock_logger.warning.assert_called_once_with("DID 9876543210 not available (partner_id != 0)")

    @pytest.mark.asyncio
    async def test_update_returns_none(self, did_repository, mock_db_manager, mock_logger):
        normalized = "919876543210"
        col = AsyncMock()
        col.find_one = AsyncMock(return_value={"did_number": normalized, "partner_id": 1})
        col.find_one_and_update = AsyncMock(return_value=None)
        _wire_connect(mock_db_manager, col)
        ...
        assert result is None
        mock_logger.warning.assert_called_with("Failed to update DID 9876543210 to partner 1")
    ...
    async def test_exception(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Unexpected error updating DID 9876543210: Database error")
...
class TestIncrementSpamCount:
    ...
    async def test_success(self, did_repository, mock_db_manager, mock_logger):
        ...
        col.update_one.assert_awaited_once_with({"did_number": did_number}, {"$inc": {"spam_count": 1}})
        assert result is True
        mock_logger.info.assert_called_with(f"Successfully incremented spam_count for DID {did_number}")
        mock_logger.error.assert_not_called()
    ...
    async def test_not_found(self, did_repository, mock_db_manager, mock_logger):
        ...
        assert result is False
        mock_logger.warning.assert_called_once_with("No document found to increment spam_count for DID 00000")
    ...
    async def test_exception(self, did_repository, mock_db_manager, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Failed to increment spam_count for DID 12345: DB error")
...
class TestGetDidsByPartner:
    ...
    @pytest.mark.asyncio
    async def test_success_no_pagination(self, did_repository, mock_db_manager, mock_logger):
        partner_id = 1
        docs = [
    ...
    @pytest.mark.asyncio
    async def test_success_with_pagination(self, did_repository, mock_db_manager, mock_logger):
        ...
        result_docs, total = await did_repository.get_dids_by_partner(partner_id, offset=offset, limit=limit)
    ...
    @pytest.mark.asyncio
    async def test_success_all_optional_filters(self, did_repository, mock_db_manager, mock_logger):
        partner_id = 1
        user_id = 200
        workspace_id = 100
        vendor_id = ObjectId()
    ...
    @pytest.mark.asyncio
    async def test_none_optional_filters_excluded(self, did_repository, mock_db_manager, mock_logger):
        cursor = self._cursor([])
        col = AsyncMock()
        ...
        await did_repository.get_dids_by_partner(1, user_id=None, vendor_id=None, workspace_id=None)
