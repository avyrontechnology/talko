...
class WebhookHandler(ABC):
    ...
    def __init__(
        ...
        Args:
            logger: HollerServiceLogger instance for logging.
            call_repository: Repository for CDR operations.
    ...
    async def process_webhook(self, payload: Dict) -> Dict:
        """
        Process a webhook payload and update the CDR.
        ...
        Raises:
            BadRequestError: If required fields are missing or update fails.
            ResourceNotFound: If the CDR is not found.
        """
        pass
    ...
    async def process_cdr_api_payload(
        ...
        """
        Process a CDR API payload and update the CDR.
    ...
    async def _relay_to_makunai(self, partner_id: int, payload: Dict[str, Any]) -> None:
        ...
        Tata's STANDARD call webhook (TataTeleWebhookHandler, type=standard),
        not the Dialer product webhook (DialerWebhookHandler, type=dialer,
        ...
        do there (Section 16.4) — and our own CDR write already succeeded
        ...
        try:
            async with httpx.AsyncClient(
                ...
                resp = await client.post(
                    ENV.MAKUNAI_CDR_WEBHOOK_URL,
                    json=relay_payload,
                    headers={"X-Webhook-Secret": ENV.CDR_WEBHOOK_RELAY_SECRET},
                )
                resp.raise_for_status()
            self.logger.debug(
                "[WebhookHandler] Relayed to makun-ai partner_id={} call_id={}".format(
                    partner_id, payload.get("call_id")
                )
            )
        except Exception as exc:
            self.logger.error(
                "[WebhookHandler] Failed to relay to makun-ai partner_id={} "
