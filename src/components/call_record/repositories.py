...
class TalkoCallRecordRepository:
    ...
    async def add_call_record(self, data: dict[str, Any]) -> str:
        async with self.__session_factory.collection(TalkoCallRecordModel.CollectionName.CALL_RECORD) as collection:
    ...
    async def is_call_record_exist(self, record_id: str, partner_id: int) -> bool:
        async with self.__session_factory.collection(TalkoCallRecordModel.CollectionName.CALL_RECORD) as collection:
            ...
            result = await collection.find_one({"_id": ObjectId(record_id), "partner_id": partner_id})
    ...
    async def update_call_record(self, record_id: str, partner_id: int, data: dict[str, Any]):
        async with self.__session_factory.collection(TalkoCallRecordModel.CollectionName.CALL_RECORD) as collection:
            await collection.update_one({"_id": ObjectId(record_id), "partner_id": partner_id}, {"$set": data})

    async def delete_call_record(self, record_id: str, partner_id: int):
        async with self.__session_factory.collection(TalkoCallRecordModel.CollectionName.CALL_RECORD) as collection:
            await collection.delete_one({"_id": ObjectId(record_id), "partner_id": partner_id})
