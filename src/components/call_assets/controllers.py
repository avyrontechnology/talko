...
class TalkoAssetsController:
    ...
    async def get_digital_assets(
        partner_id: int,
        asset_type: str,
        asset_service: TalkoAssetService = Depends(Provide[TalkoContainer.assets_service]),
        ...
        try:
            talko_service_logger.info(
                f"Fetching digital asset details for partner_id: {partner_id}, asset_type: {asset_type}"
            )
            ...
            talko_service_logger.info(
                f"Successfully fetched digital asset details for partner_id: {partner_id}, asset_type: {asset_type}"
            )
            return asset_details
        except TalkoResourceNotFound as e:
            talko_service_logger.error(f"Resource not found while getting digital assets: {str(e)}")
            return TalkoResourceNotFoundResponse(detail=NOT_FOUND)
        except TalkoBadRequestError as e:
            talko_service_logger.error(f"Bad request Error while getting digital assets: {str(e)}")
            return TalkoBadRequestResponse(detail=INVALID_REQUESTED_DATA)

        except Exception as e:
            talko_service_logger.error(
                f"Error fetching digital asset details for partner_id: {partner_id}, asset_type: {asset_type}. Error: {str(e)}"
            )
