...
class TestUserHierarchy:
    ...
    async def test_get_user_hierarchy_data_success(self, user_hierarchy, mock_grpc_client, mock_logger):
        # Arrange
        current_user_id = 100
        ...
        # Act
        result = await user_hierarchy.get_user_hierarchy_data(request_data, current_user_id)
        ...
        mock_logger.info.assert_any_call(f"Fetching user hierarchy for user_id={current_user_id}, filter_agents={[]}")

    async def test_get_user_hierarchy_data_with_filter(self, user_hierarchy, mock_grpc_client):
        # Arrange
        current_user_id = 101
        mock_grpc_client.get_user_child_details.return_value = [201]
        mock_grpc_client.get_user_child_hierarchy.return_value = {201: {"user_id": 201, "child_ids": [301]}}
    ...
    async def test_get_user_hierarchy_data_filter_excludes_all(self, user_hierarchy, mock_grpc_client):
        # Arrange
        current_user_id = 102
        mock_grpc_client.get_user_child_details.return_value = [202]
        mock_grpc_client.get_user_child_hierarchy.return_value = {202: {"user_id": 202, "child_ids": [302]}}
    ...
    async def test_get_user_hierarchy_data_no_children(self, user_hierarchy, mock_grpc_client):
        # Arrange
        current_user_id = 103
    ...
    async def test_get_user_hierarchy_data_request_data_none(self, user_hierarchy, mock_grpc_client):
        # Arrange
        current_user_id = 104
        mock_grpc_client.get_user_child_details.return_value = [204]
        mock_grpc_client.get_user_child_hierarchy.return_value = {204: {"user_id": 204, "child_ids": []}}
