...
class CurrentAuthContext(NamedTuple):
    """
...
def get_current_auth_context(request: Request) -> CurrentAuthContext:
    ...
    Raises:
        ValueError: if request.state.user is missing or partner_id cannot be
                    resolved (should not happen if AuthMiddleware ran correctly).
    """
    ...
    if is_api_key_auth:
        # API-KEY auth only ever carries partner_id (see AuthMiddleware._handle_api_key)
        user_id = None
        partner_id = current_user_data.get("partner_id")
    else:
        user_id = current_user_data.get(CurrentUserMap.USER_ID)
        partner_id = current_user_data.get(CurrentUserMap.PARTNER_ID)
