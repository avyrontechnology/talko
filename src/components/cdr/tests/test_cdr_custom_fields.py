...
class TestCDRServiceSetCustomFieldValues:
    ...
    async def test_set_custom_field_values_success(self, setup):
        ...
        custom_field_validator.validate_and_normalize_values.return_value = {"lead_source": "Web"}
        ...
        repository.set_custom_field_values.assert_awaited_once_with("call-1", {"lead_source": "Web"}, 1735689600000)
    ...
    async def test_set_custom_field_values_cdr_not_found_raises(self, setup):
        ...
        with pytest.raises(TalkoResourceNotFound):
            await service.set_custom_field_values(user_id=1, partner_id=1, call_id="missing", values={"x": 1})
    ...
    async def test_set_custom_field_values_cross_tenant_raises_not_found(self, setup):
        ...
        with pytest.raises(TalkoResourceNotFound):
            await service.set_custom_field_values(user_id=1, partner_id=1, call_id="call-1", values={"x": 1})
    ...
    async def test_set_custom_field_values_propagates_validator_errors(self, setup):
        ...
        custom_field_validator.validate_and_normalize_values.side_effect = ValueError("bad value")
    ...
    async def test_set_custom_field_values_handles_missing_updated_doc(self, setup):
        ...
        result = await service.set_custom_field_values(user_id=1, partner_id=1, call_id="call-1", values={"x": 1})
