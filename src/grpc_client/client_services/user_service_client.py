...
class TalkoUserServiceClient(TalkoGRPCClient):
    ...
    async def get_users_availability_status(self, user_ids: int | list[int]) -> dict[int, str]:
        ...
        logger.info(f"Sending GetUsersAvailabilityStatus request for user_ids: {user_ids_list}")
        ...
        try:
            ...
            logger.info(f"Received availability status for user_ids: {user_ids_list}")

            return {status.user_id: status.current_status for status in response.users_availability_status}
