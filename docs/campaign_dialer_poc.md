...
Owner service: holler-service (all campaign/lead/dialer logic, data, and APIs)
Consumer: makun-ai (calls holler-service APIs; contributes no dialer state)

---

## 1. Executive Summary

This POC adds a campaign/lead outbound dialer to holler-service, similar in spirit to
...
outbound call already works end-to-end** in the current codebase (holler-service places
...
|---|---|---|
| Place outbound call, AI-bridged, no human agent needed | Exists | `POST /holler-service/v1/call`, `enable_ai_bridge=true` — `src/components/call_management/dto.py:31` |
| Pre-create warm AI voice session while ringing | Exists | `_pre_create_session()` → `POST /ai/v1/voice/sessions` (makun-ai) — `src/components/call_management/services.py:397-418` (holler-service), `src/components/voice/controllers.py:152` (makun-ai) |
| CDR with campaign_id, lead_id, disposition, llm_analysis | Exists | `src/components/cdr/models.py` |
| DID pool with AI vs. human split | Exists | `PhoneNumberManagement.did_type` = `NORMAL` \| `AI_AGENT` — `src/components/did_management/models.py:26` |
...
Note: holler-service's operational datastore is **MongoDB** (via Motor/`AsyncIOMotorClient`,
`src/core/doc_db.py`), matching the pattern used by `CDR` and `PhoneNumberManagement`. The
...
- **holler-service** owns all campaign/lead/dialer state, business logic, and background
  workers. It is the system of record.
- **makun-ai** is a pure API consumer: it calls holler-service to create campaigns, upload
  leads, control lifecycle, and pull reports. Its only role in call execution is the
  existing `/ai/v1/voice/sessions` endpoint, called internally by holler-service exactly
...
makun-ai (UI/backend)
   ...
   ▼
holler-service — new "campaign" module (MongoDB via Motor, same convention as CDR/DID)
   │
   ...
   │
   └─ call_type = "ai" ─────► holler-owned dispatch engine:
          ...
          → pre-created makun-ai voice session bridges in automatically
                    │
                    ▼
   Both paths converge: existing /call/webhook?type=dialer → CDR upsert
                    │
                    ▼
   Campaign/Lead status sync job reads CDR → updates Lead.status, schedules retries
                    │
                    ▼
...
and abandon-rate compliance ourselves — that infra does not exist in holler-service today
(confirmed: `call_agent_map` only does static agent↔DID mapping, no live presence/status)
and is expensive to build correctly. For AI campaigns, holler-service owns the full dispatch
...
Follows the existing `TimestampedModel` / `CollectionName` convention used by `CDR` and
...
class Lead(TimestampedModel):
    ...
    last_call_uuid: Optional[str]       # joins into existing CDR.call_uuid

    class CollectionName:
        LEAD = "lead"
```

No separate `CallAttempt` collection — `CDR` already carries `campaign_id`, `lead_id`,
...
- **Dedicated Celery queue** `campaign_dialer_queue`, following the existing `task_routes`
  convention in `src/holler_celery/celery_config.py`, scaled independently of
  `call_operation_queue` / `did_management_queue` / `call_assets_queue` so campaign bursts
  don't starve CDR polling or DID-cooldown tasks.
...
- **DID allocation**: round-robin over `PhoneNumberManagement` where `did_type=AI_AGENT`,
...
`/call/webhook?type=dialer` → CDR (`is_incoming_from_broadcast`) → `Lead.status` sync —
no new webhook handling required.

## 9. API Surface (`/holler-service/v1/campaigns`) — consumed by makun-ai
...
holler-service endpoint.

## 10. DID Management Strategy

- Reuse the existing `PhoneNumberManagement` model as-is — no schema change needed.
...
- **Idempotency**: per-lead Redis lock plus `task_acks_late=True` (already the holler-service
...
- Per-campaign metrics: dispatch rate, connect rate, error rate, cost burn — all derivable
  from existing CDR fields plus the new Redis counters, no new observability stack needed.
...
- Call placement: `POST /holler-service/v1/call` (`enable_ai_bridge`).
- AI voice session bridging: `POST /ai/v1/voice/sessions` (makun-ai).
- CDR ingestion and disposition capture: existing `/call/webhook?type=dialer` handler.
- Human-agent predictive dialing: Tata Tele's native broadcast list feature via the
  existing `dialer` module.
- DID pool model and cooldown/spam logic: existing `PhoneNumberManagement`.
- Celery queue/task-routing conventions: existing `src/holler_celery/celery_config.py`
  pattern.
